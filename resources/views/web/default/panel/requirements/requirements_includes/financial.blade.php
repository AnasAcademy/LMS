<section class="mt-30">
    {{-- @if (!empty($registrationBonusAmount))
        <div class="mb-25 d-flex align-items-center justify-content-between p-15 bg-white panel-shadow">
            ...
            <a href="/panel/marketing/registration_bonus"
                class="btn btn-border-gray300">{{ trans('update.view_more') }}</a>
        </div>
    @endif --}}
    ...
    {{-- bundle details --}}
    <div>
        <h1 class="mt-5 font-36">بيانات الدبلومه</h1>
<div class="d-flex align-items-center">
    <h2 class="text-primary">الدبلومة المسجل اليها: </h2>
    <p class="font-20 my-2 mr-3">
        {{ $bundle->category->slug }}
    </p>
</div>

<div class="d-flex align-items-center">
    <h2 class="text-primary"> عدد الأيام المتاحه لمشاهدة الدبلومه: </h2>
    <p class="font-20 my-2 mr-3">
        {{ $bundle->access_days }} يوم
    </p>
</div>

<div class="d-flex align-items-center">
    <h2 class="text-primary"> المحاضر: </h2>
    <p class="font-20 my-2 mr-3">
        ...
        <form
            ...
            <div class="row">
                @foreach ($paymentChannels as $paymentChannel)
                    @if (!$isMultiCurrency or !empty($paymentChannel->currencies) and in_array($userCurrency, $paymentChannel->currencies))
                        <div class="col-6 col-lg-3 mb-40 charge-account-radio">
                            <input type="radio" class="online-gateway" name="gateway"
                                id="{{ $paymentChannel->class_name }}" @if (old('gateway') == $paymentChannel->class_name) checked @endif
                                value="{{ $paymentChannel->class_name }}">
                            <label for="{{ $paymentChannel->class_name }}"
                                ...
                                <p class="mt-30 font-14 font-weight-500 text-dark-blue">
                ...
                @if (!empty(getOfflineBankSettings('offline_banks_status')))
                    <div class="col-6 col-lg-3 mb-40 charge-account-radio">
                        ...
                        <label for="offline"
                            class="rounded-sm p-20 p-lg-45 d-flex flex-column align-items-center justify-content-center">
                            <img src="/assets/default/img/activity/pay.svg" width="120" height="60"
                                alt="">
            ...
            @if (!empty($invalidChannels))
                <div class="d-flex align-items-center rounded-lg border p-15">
                    ...
                    <div class="ml-5">
                        <h4 class="font-14 font-weight-bold text-gray">{{ trans('update.disabled_payment_gateways') }}
                        </h4>
            ...
            <div class="">
                ...
                <div class="row mt-3">
                    <div class="col-12 col-lg-3 mb-25 mb-lg-0">
                        <label
                            class="font-weight-500 font-20 text-primary d-block">{{ 'المبلغ المراد دفعه ' }}</label>
                        <div class="input-group">
                            ...
                            {{-- bundle info --}}
                            <input type="number" name="amount" min="0" readonly
                                class="form-control @error('amount') is-invalid @enderror font-24"
                                value="{{ $bundle->price }}" placeholder="{{ trans('panel.number_only') }}" />
                    ...
                    <div class="col-12 col-lg-3 mb-25 mb-lg-0 js-offline-payment-input "
                        ...
                        <div class="form-group">
                            <label for="referralCode"
                                class="input-label">{{ trans('admin/main.referral_code') }}</label>
                    ...
                    <div class="col-12 col-lg-3">
                        <div class="mt-30">
                            <button type="button" id="submitChargeAccountForm"
                                class="btn btn-primary btn-sm">{{ trans('public.pay') }}</button>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <table class="table text-center custom-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ trans('panel.amount') }} ({{ $currency }})
                                        </th>
