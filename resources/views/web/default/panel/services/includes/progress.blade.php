<div class="webinar-progress d-block d-lg-flex align-items-center p-15 panel-shadow bg-white rounded-sm">

    <div class="progress-item d-flex align-items-center">
        <a href="/panel/services" style="width:55px" class="p-10 d-flex align-items-center justify-content-center active"
            data-toggle="tooltip" data-placement="top">
          @include('web.default.panel.includes.sidebar_icons.requests')


        </a>

        <div class="ml-10' }}">
            <h4 class="font-16 text-secondary font-weight-bold">الخدمات الإلكترونية</h4>
        </div>
    </div>
</div>
