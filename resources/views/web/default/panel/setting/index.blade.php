    ...
    <form method="post" id="userSettingForm" class="mt-30"
        ...
        @if (empty($new_user) and !empty($currentStep))
            @switch($currentStep)
                ...
                @case(6)
                    @if ($user->student)
                        @include('web.default.panel.setting.setting_includes.relevant_data')
                    @endif
                @break
        ...
        <div class="d-flex align-items-center">
            @if (!empty($user) and empty($new_user))
                ...
                <button type="button" id="getNextStep" class="btn btn-sm btn-primary ml-15"
                    @if (!empty($currentStep) and ($user->isUser() and $currentStep == 7 or !$user->isUser() and $currentStep == 9)) disabled @endif>{{ trans('webinars.next') }}</button>
            @endif
        </div>

        <div class="d-flex align-items-center">
            @if (empty($new_user) and empty($edit_new_user))
                <a href="/panel/setting/deleteAccount" class="delete-action btn btn-sm btn-danger"
