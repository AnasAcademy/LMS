...
// dd($carts);
    ...
    public function createOrderAndOrderItems($carts, $calculate, $user, $discountCoupon = null)
    {
        ...
        foreach ($carts as $cart) {
            ...
            OrderItem::create([
                ...
                'certificate_template_id' => $cart->certificate_template_id ?? null,
                'certificate_bundle_id' => $cart->certificate_bundle_id ?? null,
    ...
    private function getSeller($cart)
    {
        ...
        if (!empty($cart->webinar_id) or !empty($cart->bundle_id) or !empty($cart->certificate_template_id)) {
            // $user = $cart->webinar_id ? $cart->webinar->creator : $cart->bundle->creator;
             $user = (!empty($cart->webinar_id)) ? $cart->webinar->creator : ((!empty($cart->bundle_id)) ? $cart->bundle->creator : null);
    ...
    public function handleOrderPrices($cart, $user, $taxIsDifferent = false, $discountCoupon = null)
    {
        ...
        if (!empty($cart->webinar_id) or !empty($cart->bundle_id) or !empty($cart->certificate_template_id)) {
            ...
            $item = !empty($cart->webinar_id) ? $cart->webinar : (!empty($cart->bundle_id) ? $cart->bundle :  $cart->certificate_template);

            $price = $item->price;
            if(!empty($cart->certificate_template))
            {
                $discount=null;
            }
            else{
