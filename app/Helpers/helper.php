...
{
    return getFinancialCurrencySettings('currency') ?? 'SAR';
}
    ...
    if (!empty($notificationTemplate)) {
        ...
        if (!empty($options['[c.title]'])) {
            ...
            if ($options['[c.title]'] == "رسوم حجز مقعد دراسي" && !empty($options['[c.early]'])) {
                ...
                يرجى ملاحظة أن التسجيل الرسمي سيبدأ في شهر يناير 2025. بمجرد فتح التسجيل، ستتمكن من استكمال رفع المتطلبات اللازمة وإتمام إجراءات التسجيل. نرحب بك في برنامجنا ونتطلع معًا إلى فترة دراسية ناجحة معنا" : 1;
            }
